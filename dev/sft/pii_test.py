#!/usr/bin/env python3
"""
PII Redaction Benchmark using ART.
...
Usage:
    ...
    uv run python dev/sft/pii_test.py --base-model

    # Test fine-tuned model
    uv run python dev/sft/pii_test.py
"""
...
async def run_benchmark(
    ...
    Args:
        model: A registered TrainableModel to evaluate
...
async def main():
    parser = argparse.ArgumentParser(description="PII Redaction Benchmark with ART")
    parser.add_argument(
        "--test-file",
    ...
    parser.add_argument(
        "--base-model",
        action="store_true",
        help="Test the base model without fine-tuning (baseline)",
    )
    parser.add_argument(
        "--max-entries",
        type=int,
    ...
    if args.base_model:
        # Use base model without any fine-tuning (unique name = no checkpoints)
        model = art.TrainableModel(
            name="qwen-2.5-7b-baseline-eval",
            project="pii-redaction",
